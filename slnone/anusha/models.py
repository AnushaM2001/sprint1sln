...
def validate_amount(value):
    # Ensure the amount is a valid number and does not exceed 10 digits
    if not isinstance(value, (int, Decimal)):
        raise ValidationError('Amount must be a valid number.')
    if len(str(abs(value))) > 10:
        raise ValidationError('Amount must be less than 10 digits.')
...
class LoanApplication(models.Model):
    ...
    co_applicant_mobile_number = models.CharField(max_length=15, null=False, blank=False, validators=[validate_mobile_number],default="")
    ...
    co_applicant_net_income_per_month = models.IntegerField(null=False, blank=False,validators=[validate_amount],default=10000)
